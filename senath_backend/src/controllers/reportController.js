...
const createReport = async (req, res) => {
    try {
        const userId = 1;
        const { report_type, issue_type, details, image_path } = req.body;
        ...
        const [result] = await db.query(
            'INSERT INTO reports (user_id, report_type, issue_type, details, image_path) VALUES (?, ?, ?, ?, ?)',
            [userId, report_type, issue_type, details, image_path]
        );
